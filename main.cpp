...
void filtrage(const Donnees &donnees, Mat_indice &entree) {
    ...
    for (int n_filtr(1); n_filtr <= donnees.nbF; ++n_filtr) {
        for (size_t li(1); li < donnees.li - 1; ++li) { //Parcours des lignes
            for (size_t col(1);col<donnees.col-1;++col) { //Parcours de colonnes
                if(entree[li][col]==0){ //Un pixel noir le restera
                    destination[li][col]=0;
                    continue;
                }
